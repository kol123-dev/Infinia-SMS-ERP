    ...
    function teacherAccess()
    {
        try {
            ...
            if ($user->role_id == 4) {
                ...
                if ($teacher_info) {
                    $has_assigned_classes = SmAssignSubject::where('teacher_id', $teacher_info->id)
                        ->where('school_id', $user->school_id)
                        ->where('academic_id', getAcademicId())
                        ->exists();
                    return $has_assigned_classes;
                }
                return false;
            } else {
                return false;
...
if (!function_exists('teacherHasAccessToStudent')) {
    function teacherHasAccessToStudent($student_id)
    {
        try {
            if (!teacherAccess()) {
                return false;
            ...
            // Check if teacher is assigned to student's class and section
            $has_access = SmAssignSubject::where('teacher_id', $teacher_info->id)
...
if (!function_exists('teacherHasAccessToClass')) {
    function teacherHasAccessToClass($class_id, $section_id = null)
    {
        try {
            if (!teacherAccess()) {
                return false;
            ...
            $query = SmAssignSubject::where('teacher_id', $teacher_info->id)
                ->where('class_id', $class_id)
                ->where('school_id', $user->school_id)
                ->where('academic_id', getAcademicId());
